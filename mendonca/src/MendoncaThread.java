...
public class MendoncaThread extends Object implements Runnable {
	...
	public void run() {
		...
		try{
			...
			double symmetric_features = MendoncaThread.calcSymmetricFeatures(featureModel.getRoot(),featureModel);
			symmetric_features = symmetric_features + MendoncaThread.calcSymmetricConstraints(featureModel);
...
//			output = output + "\t" + String.valueOf(symmetric_features/totalfeatures*100);
		...
		{
			System.err.println("Interruped process by timeout");
		}
		catch(Exception ex)
		{
